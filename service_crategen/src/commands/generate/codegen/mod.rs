...
pub trait GenerateProtocol {
    ...
    ///
    /// The method generated by this method are inserted into an enclosing `impl Foo for FooClient {}` block
    fn generate_method_impls(&self, writer: &mut FileWriter, service: &Service) -> IoResult;

    /// Generate a method for each `Operation` in the `Service` to forward that method through
    /// a wrapper type (e.g. `Rc`).
    ///
    /// The methods generated by this method are inserted into an enclosing `impl<T: Foo> Foo for Wrapper<T> {}` block
    ...
    where P: GenerateProtocol
{
    ...
    protocol_generator.generate_forwarding_method_impls(writer, service)?;
    writeln!(writer, "}}")
}
